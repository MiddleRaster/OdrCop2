...
namespace SomeNamespace
{
    inline namespace V2 { int foo() { return 42; } }
           namespace V1 { int foo() { return 41; } }
            ...
            Assert::AreEqual("SomeNamespace::V1::foo()", (*it++).first, "should return the key");
            Assert::AreEqual("SomeNamespace::foo()",     (*it++).first, "should return the key");
        }
    },
...
void process(Helper h) { (void)h; }
            ...
            Assert::AreEqual("process((anonymous namespace in input.cc)::Helper)", maps.functionMap.begin()->first, "should return the key");
        }
    },
...
Outer makeOuter() { return Outer{}; }   // Function returning a type containing an anonymous struct
            ...
            const auto& vec = maps.functionMap.begin()->second;
            Assert::AreEqual(1, vec.size(), "should have found 1 function");
            Assert::AreEqual("makeOuter()", maps.functionMap.begin()->first, "should return the key");
        }
    },
...
Variant makeVariant() { return Variant{}; }
            ...
            const auto& vec = maps.functionMap.begin()->second;
            Assert::AreEqual(1, vec.size(), "should have found 1 function");
            Assert::AreEqual("makeVariant()", maps.functionMap.begin()->first, "should return the key");
        }
    },
...
struct Pixel
{
    ...
    uint32_t getPacked() const { return packed; }
            ...
            const auto& vec = maps.functionMap.begin()->second;
            Assert::AreEqual(1, vec.size(), "should have found 1 function");
            Assert::AreEqual("Pixel::getPacked() const", maps.functionMap.begin()->first, "should return the key");
        }
    },
};
...
Test ExploringOdrViolationReportingTests[] =
{
    ...
    {"Report typedef/using ODR violations", []
        {
            ...
            Assert::AreEqual(true, clang::tooling::runToolOnCodeWithArgs(std::make_unique<OdrCop2::VisitorAction>(maps), code1, { "-x", "c++", "-std=c++23" }, "tu1.cpp"));
            Assert::AreEqual(true, clang::tooling::runToolOnCodeWithArgs(std::make_unique<OdrCop2::VisitorAction>(maps), code2, { "-x", "c++", "-std=c++23" }, "tu2.cpp"));
            Assert::AreEqual(true, clang::tooling::runToolOnCodeWithArgs(std::make_unique<OdrCop2::VisitorAction>(maps), code2, { "-x", "c++", "-std=c++23" }, "tu3.cpp"));
    ...
    {"Report UDT ODR violations", []
        {
            std::string code1 = "struct S { int  x; };";
            std::string code2 = "struct S { long x; };";

            OdrCop2::AllMaps maps;
            Assert::AreEqual(true, clang::tooling::runToolOnCodeWithArgs(std::make_unique<OdrCop2::VisitorAction>(maps), code1, { "-x", "c++", "-std=c++23" }, "tu1.cpp"));
            Assert::AreEqual(true, clang::tooling::runToolOnCodeWithArgs(std::make_unique<OdrCop2::VisitorAction>(maps), code2, { "-x", "c++", "-std=c++23" }, "tu2.cpp"));

            Assert::AreEqual(1, maps.udtMap.size(), "should be only one UDT name");
